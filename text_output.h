...
public:
	...
	void write(int font, const string& text, float size, float x0,float y0, float x1, float y1, bool inverse_color, float r, float g, float b, float alpha);
	void find_letter_width(char kirjain, int *nume, int *kirjainleveys);
	int write_line(int font, float x, float y, const string& text, float size);
	bool accept_messages;
	void message(float timer, float fade_time, const string& message);
